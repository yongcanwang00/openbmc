...
static uint8_t get_6bit_ascii(char c)
{
    return (c - 0x20) & 0x3f;
}

static uint8_t get_aligned_size(uint8_t size, uint8_t align)
{
...
{
    char *data = NULL;
    struct fru_type_length *ftl = NULL;
...
int gen_bia(dictionary *ini, char **bia_data)
{
    ...
    if (mfg_date == -1) {
        fprintf(stdout, "Manufacturing time not specified. "
                "Set current time as Manufacturing time\n");
        gettimeofday(&tv, NULL);
        mfg_date = tv.tv_sec - secs_from_1970_1996;
        mfg_date = mfg_date/60;
    }
